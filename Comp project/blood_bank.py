...
#mycursor.execute("Select*FROM blood")
#mycursor1.execute("create table admin(admin_name varchar(255), admin_password varchar(255)")
#sql = "INSERT INTO admin (admin_name,admin_password)
#mycursor1.execute("insert into admin values ('Vidisha',654321)")
#val = [
...
def privpol():
    ...
    about =tk.Label(topa,text = 'Privacy policies:' ,fg="black",bg = 'red',font=("Helvetica", 20,"bold"))
...
def main():
    ...
    btn.place(x=600,y=125)
    btn = tk.Button(f, text = 'ABOUT US', command = abtus, fg = "White" ,bg="Black", width="15")
    btn.place(x=800,y=125)
    btn = tk.Button(f, text = 'PRIVACY POLICY', command = privpol, fg = "White" ,bg="Black", width="15")
    btn.place(x=1000,y=125)
    btn = tk.Button(f, text = 'BLOOD TIPS', command =btip, fg = "White" ,bg="Black", width="15")
    btn.place(x=1200,y=125)           
    main.mainloop()
...
def signup():   
    def dialog2():
        ...
        for x in l:
          ...
          if ((len(str(mob))>10 or (len(str(mob))<=9))):
              flagg = 5
            
          if ( (x[1] != fname or fname == "") and len (password)>4 ):
              flag = 2
              
          if ((len(fname)<4) and (len(password)<=2)):
              flag = -1
        ...
        if (flag == 1):
            ...
            box.showinfo("info","The length of Password name must be minimum of 5 characters and the length of the password must be greater than 2 character")
    ...
    labelb = tk.Label(f,text="Bangalore",width=19,font=("bold", 12))
